...
# ============================================================================
# v2.0 BATCH RUN — Drive persistence (Step B revB, 2026-04-28)
# ----------------------------------------------------------------------------
# Edit FEATURE_LIST and ASPECT_FILE below. ALL features in FEATURE_LIST must
# live in the SAME aspect_file. To process features from multiple coordinate
# frames, run multiple batches (one per aspect file).
#
# Output is written directly to the Drive workspace. A pre-run snapshot is
# saved BEFORE engine mutation and a post-run snapshot is saved AFTER, with
# deduplication applied to the registry and temporal-persistence CSVs
...
def _save_snapshot(label: str, manifest_lines: list[str]) -> Path:
    ...
    run_id = datetime.datetime.now(datetime.UTC).strftime("%Y%m%d_%H%M%S_%f")
...
print("="*78)
_save_snapshot('pre', [
    f"Trigger: batch run (pre-mutation)",
    f"Aspect file: {ASPECT_FILE}",
    f"Feature count (planned): {len(FEATURE_LIST)}",
    f"Features: {FEATURE_LIST}",
    f"Workspace: {WORKSPACE}",
])

print("\n" + "="*78)
print(f"V2.0 BATCH RUN | {len(FEATURE_LIST)} features")
print(f"  base_file   : {BASE_FILE}")
print(f"  aspect_file : {ASPECT_FILE}")
print(f"  output_dir  : {OUTPUT_DIR}")
print("="*78)

manifest = run_v2_batch(BASE_FILE, ASPECT_FILE, FEATURE_LIST, OUTPUT_DIR)

print("\n" + "="*78)
print("BATCH COMPLETE")
print(f"  succeeded : {manifest['feature_count_succeeded']}")
print(f"  skipped   : {manifest['feature_count_skipped']}")
if manifest['skipped_features']:
    for s in manifest['skipped_features']:
        print(f"     - {s['feature']}: {s['error']}")
...
print("="*78)
_save_snapshot('post', [
    f"Trigger: batch run (post-dedupe)",
    f"Aspect file: {ASPECT_FILE}",
    f"Feature count: {len(FEATURE_LIST)} requested, "
    f"{manifest['feature_count_succeeded']} succeeded, "
    f"{manifest['feature_count_skipped']} skipped",
    f"Features: {FEATURE_LIST}",
    f"Workspace: {WORKSPACE}",
])
